...
function withFallback(mockOperation, supabaseOperation) {
  return async (...args) => {
...
const supabaseGetDirectoryTree = async () => {
...
const supabaseGetArticle = async (id) => {
...
const mockCreateDirectory = (directory) => {
...
const supabaseCreateDirectory = async (directory) => {
  const { data, error } = await supabase
    .from('directories')
    .insert(directory)
    .select()
...
const mockUpdateDirectory = (id, updates) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
...
const supabaseUpdateDirectory = async (id, updates) => {
  const { data, error } = await supabase
    .from('directories')
    .update(updates)
    .eq('id', id)
...
const supabaseDeleteDirectory = async (id) => {
...
const mockCreateArticle = (article) => {
  return new Promise(resolve => {
    setTimeout(() => {
      const newArticle = {
        id: Date.now().toString(),
...
const supabaseCreateArticle = async (article) => {
  const { data, error } = await supabase
    .from('articles')
    .insert(article)
    .select()
...
const mockUpdateArticle = (id, updates) => {
  return new Promise(resolve => {
    setTimeout(() => {
...
const supabaseUpdateArticle = async (id, updates) => {
  const { data, error } = await supabase
    .from('articles')
    .update(updates)
    .eq('id', id)
...
const supabaseDeleteArticle = async (id) => {
  const { error } = await supabase
    .from('articles')
    .delete()
    .eq('id', id)
...
const mockSearchArticles = (query) => {
  return new Promise(resolve => {
    setTimeout(() => {
...
const supabaseSearchArticles = async (query) => {
  if (!query.trim()) {
    const { data, error } = await supabase
      .from('articles')
      .select(`
